...
//   (selector moved to OFF, OR a manual-button long-press).
//
...
//
//   A manual long-press acknowledge is an operator OVERRIDE that clears ALL
//   latched bits once the healthy-hold has elapsed -- including manual-clear-only
//   bits such as FAULT_INTERNAL.
// =============================================================================
...
Mode decodeMode(bool aClosed, bool bClosed) {
  // INPUT_PULLUP active-low; common to GND 3-position selector.
  //  open/open  (1,1) -> OFF    (also the unwired/disconnected safe default)
  //  closed/open(0->A) -> MANUAL
  //  open/closed(B)    -> AUTO
  //  closed/closed     -> INVALID (electrically impossible for a healthy selector)
  if (!aClosed && !bClosed) return Mode::OFF;
  if (aClosed && !bClosed)  return Mode::MANUAL;
  if (!aClosed && bClosed)  return Mode::AUTO;
  return Mode::INVALID;
}

// Compute the set of fault conditions present THIS tick (not yet latched).
uint16_t detectFaults(const Inputs& in, Mode decoded) {
  ...
  if (decoded == Mode::INVALID) f |= FAULT_MODE_SELECTOR;
...
bool conditionHealthy(uint16_t bit, const Inputs& in, Mode decoded) {
  switch (bit) {
    ...
    case FAULT_MODE_SELECTOR: return decoded != Mode::INVALID;
...
bool allRecoverableHealthy(uint16_t faults, const Inputs& in, Mode decoded) {
  for (uint16_t bit = 1; bit != 0; bit <<= 1) {
    if ((faults & bit) && !isManualClearOnly(bit) && !conditionHealthy(bit, in, decoded)) {
      return false;
    }
  }
  return true;
...
Decision evaluate(State& s, const Inputs& in, uint32_t nowMs) {
  ...
  Mode decoded = decodeMode(in.aClosed, in.bClosed);

  // --- 1) Fault detection + latch ------------------------------------------
  uint16_t live = detectFaults(in, decoded);
  s.faults |= live;  // latch new faults

  // --- 2) Recovery gate -----------------------------------------------------
  // Track how long all SELF-RECOVERABLE latched conditions have been healthy.
  //  - FAULT_PV_INVALID is clear-on-recovery (no ack): a transient PV rail must
  //    not require a human to re-arm unattended AUTO.
  //  - All other self-recoverable faults clear on healthy-hold + operator ack
  //    (selector to OFF or manual long-press).
  ...
  if (s.faults != FAULT_NONE) {
    bool healthyNow = allRecoverableHealthy(s.faults, in, decoded);
    if (healthyNow) {
      if (!s.wasHealthy) {
      ...
      bool ack = in.buttonLong || (decoded == Mode::OFF);
  ...
  // --- 3) FAULT priority ----------------------------------------------------
  if (s.faults != FAULT_NONE) {
    s.mode = Mode::INVALID;  // reflect that no normal mode is active
    resetAuto(s);
    d.activeMode = Mode::INVALID;
  ...
  // No faults: track mode transitions and reset per-mode latches on entry.
  if (decoded != s.mode) {
    if (decoded == Mode::MANUAL) s.manualOn = false;  // MANUAL always (re)enters OFF
    if (decoded == Mode::AUTO)   resetAuto(s);        // AUTO re-enters with load OFF view
    s.mode = decoded;
  }
  ...
  if (s.mode == Mode::MANUAL) {
    if (in.buttonPressed) {
      s.manualOn = !s.manualOn;  // toggle on each clean press
    }
    desired = s.manualOn;
    d.reason = s.manualOn ? "manual on" : "manual off";
    // dwellGated stays false: a manual press takes effect immediately.
  }
  // --- 6) AUTO priority -----------------------------------------------------
    ...
    // Genuinely unreachable: INVALID is handled as a fault above, and decoded is
    // one of OFF/MANUAL/AUTO/INVALID. Reaching here means s.mode holds an
    // impossible value -> latch an internal state error and force OFF this tick.
